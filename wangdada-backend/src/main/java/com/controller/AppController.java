...
import com.model.dto.app.AppEditRequest;
import com.model.dto.app.AppQueryRequest;
import com.model.dto.app.AppUpdateRequest;
...
public class AppController {
    ...
    @PostMapping("/review")
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<Boolean> doReviewApp(@RequestBody ReviewRequest reviewRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(reviewRequest == null, ErrorCode.PARAMS_ERROR);
        Long id = reviewRequest.getId();
        Integer reviewStatus = reviewRequest.getReviewStatus();
        //校验
        ReviewStatusEnum statusEnum = ReviewStatusEnum.getEnumByValue(reviewStatus);
        if (statusEnum== null || id ==null) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        App app = appService.getById(id);
        ThrowUtils.throwIf(app == null, ErrorCode.NOT_FOUND_ERROR);
        //已经是该状态
        ThrowUtils.throwIf(Objects.equals(reviewStatus, app.getReviewStatus()), ErrorCode.PARAMS_ERROR,"请勿重复审核");
        //更新审核状态
