...
import com.model.enums.AppScoringStrategyEnum;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public void validApp(App app, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(AppScoringStrategyEnum.getEnumByValue(scoringStrategy) == null, ErrorCode.PARAMS_ERROR, "应用评分策略非法");
        }
        // 修改数据时，有参数则校验
        if (StringUtils.isNotBlank(appName)) {
            ThrowUtils.throwIf(appName.length() > 80, ErrorCode.PARAMS_ERROR, "应用名称过长");
            ThrowUtils.throwIf(appDesc.length() > 300, ErrorCode.PARAMS_ERROR, "应用描述过长");
        }
        if (reviewStatus != null) {
            ThrowUtils.throwIf(ReviewStatusEnum.getEnumByValue(reviewStatus) == null, ErrorCode.PARAMS_ERROR, "应用审核状态非法");
        }
    }
    ...
    public QueryWrapper<App> getQueryWrapper(AppQueryRequest appQueryRequest) {
        ...
        if (StringUtils.isNotBlank(searchText)) {
            ...
            queryWrapper.and(qw -> qw.like("appName", searchText)
                    .or().like("reviewMessage", searchText)
                    .or().like("appDesc", searchText)
            );
        }
        // 模糊查询
        queryWrapper.like(StringUtils.isNotBlank(appName), "appName", appName);
        queryWrapper.like(StringUtils.isNotBlank(appDesc), "appName", appName);
        queryWrapper.like(StringUtils.isNotBlank(reviewMessage), "appName", appName);
        // 精确查询
